...
/**
 * A class interface that allows for easy interaction with NinjaCoind
 */
export class NinjaCoind extends HTTPClient implements NinjaCoindTypes.INinjaCoind {
    /**
    ...
    public async transactionsStatus (
        transactions: string[]
    ): Promise<NinjaCoindTypes.ITransactionsStatus> {
    ...
    public async randomIndexes (
        amounts: number[],
        count = 3
    ): Promise<NinjaCoindTypes.IRandomOutput[]> {
    ...
    public async indexes (
        startHeight: number,
        endHeight: number
    ): Promise<NinjaCoindTypes.ITransactionIndexes[]> {
        return this.get('indexes/' + startHeight + '/' + endHeight)
            .then((result: NinjaCoindTypes.ITransactionIndexes[]) =>
    ...
    public async sync (
        ...
        count = 100): Promise<NinjaCoindTypes.ISync> {
        ...
        response.blocks = response.blocks.map((block: NinjaCoindTypes.ISyncBlock) => {
    ...
    public async rawSync (
        ...
        timestamp = 0,
        skipCoinbaseTransactions = false,
        count = 100): Promise<NinjaCoindTypes.IRawSync> {
